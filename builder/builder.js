...
var build_project = require('./build.js');
...
window.addEventListener("load",function(){
	...
	window.addEventListener("click",function(e){
		...
		if(is(e,'#runprojectsudo') && e.target.getAttribute('disabled') != 'disabled'){
			runAppAsSudo();
		}
		...
		if(e.target.classList.contains('tab')){
			displayTab(e.target.getAttribute('tabid'));
		}
	});
...
function runAppAsSudo(v,pkg){
	dirbin=path.basename(genDL(build.version,os.type(),os.arch(),true));
	build.runing=true;
	updateState();
	cxmd="gksu builder/bin/"+dirbin+"/nw '"+app.workdir+"/' "+getID('params').value;
	_console.dom.innerHTML+="<span style='color: #2082ac;'>=======<br>Run app as sudo<br>=======<br><br>"+cxmd+"</span><br><br>";
	child=exec(cxmd);
	build.pids.push(child.pid);
	child.on('exit',function(){
		_console.dom.innerHTML+="<span style='color: #2082ac;'>==============<br>Exit with code:"+child.exitCode+"<br>==============</span><br>";
...
function displayTab(tabid){
	tabcont=document.getElementById('container');
	tabs=tabcont.querySelectorAll('.tab');
	sections=tabcont.querySelectorAll('.section');
	for (var i = 0; i < tabs.length; i++) {
		if(tabs[i].getAttribute('tabid') == tabid){
			tabs[i].setAttribute('sel','1');
		}else{
			tabs[i].setAttribute('sel','0');
		}
	};
	for (var i = 0; i < sections.length; i++) {
		if(sections[i].getAttribute('tabid') == tabid){
			sections[i].setAttribute('sel','1');
		}else{
			sections[i].setAttribute('sel','0');
		}
	};
}
function buildApp(){
	...
	consoleAddMsg("Adding project files<br>");
	build_project.build(app.workdir,"builder/bin/"+dirbin+"/nw",app.workdir+"/build/",function(){
		consoleAddMsg("Merge nw with aplication data<br>");
		dirbin=path.basename(genDL(build.version,os.type(),os.arch(),true));
		consoleAddMsg("Copying nwjs default files<br>")
	...
	if(!fs.existsSync(app.workdir+'/.build')){
		...
		fs.mkdirSync(app.workdir+'/.build');
		build_project.build(app.workdir,"builder/bin/"+dirbin+"/nw",app.workdir+"/build/");
		//copyFiles();
	}*/
}
...
function updateState(){
	...
	if((!build.runing || app.package["single-instance"] == false || app.package["ml"] == true) && build.version != "" && app.workdir != "" && fs.existsSync('builder/bin/'+dirbin)){
		getID('runproject').setAttribute('disabled','false');
		getID('runprojectsudo').setAttribute('disabled','false');
		getID('build').setAttribute('disabled','false');
	}else{
		getID('runproject').setAttribute('disabled','disabled');
		getID('runprojectsudo').setAttribute('disabled','disabled');
...
function unpk_command(p){
	...
	if(lc=="win" || lc=="Windows_NT"){command="z7.exe x ";}
	return command;
}
